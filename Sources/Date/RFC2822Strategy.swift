...
///
/// `@RFC2822Date` decodes RFC 2822 date strings into `Date`s. Encoding the `Date` will encode the value back into the
...
public struct RFC2822Strategy: DateValueCodableStrategy {
    ...
    public static func encode(_ date: Date) -> String {
        RFC2822Strategy.dateFormatter.string(from: date)
    }
}
